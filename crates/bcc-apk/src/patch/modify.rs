...
impl ApkEditor {
    pub fn from_paths(manifest_path: &Path, table_path: Option<&Path>) -> Result<Self, ResError> {
        ...
        let manifest = XMLTree::read(&mut manifest_file)
    ...
    pub fn save_to_paths(self, manifest_path: &Path, table_path: Option<&Path>) -> Result<(), ResError> {
        ...
        self.manifest.write(&mut manifest_output_file)
            .map_err(|error| ResError::Manifest(error.to_string()))?;
        ...
        if let (Some(target_path), Some(resource_table)) = (table_path, self.res_table) {
            ...
            resource_table.write_all(&mut table_output_file)
                .map_err(|error| ResError::Manifest(error.to_string()))?;
        }
        Ok(())
    }
    ...
    pub fn get_current_package(&mut self) -> Option<String> {
        let root_element = self.manifest.root.get_element_mut(&["manifest"], &self.manifest.string_pool)?;
    ...
    pub fn apply_patches(&mut self, target_package_suffix: &str, app_title: &str) -> Result<(), ResError> {
        ...
        root_element.children.retain(|child_node| {
            let resolved_name = child_node.element.name.resolve(&self.manifest.string_pool).unwrap_or_default();
            resolved_name != "split"
        });

        root_element.element.attributes.retain(|attribute_node| {
            let Some(resolved_name) = attribute_node.name.resolve(&self.manifest.string_pool) else { return true; };
        ...
        replace_package_references(&mut self.manifest.root, &mut self.manifest.string_pool, resource_table_reference, &original_package_name, &final_constructed_package_name);

        let Some(application_element) = self.manifest.root.get_element_mut(&["manifest", "application"], &self.manifest.string_pool) else {
            return Ok(());
        };
        ...
        application_element.children.retain(|child_node| {
            ...
            if !is_metadata_tag { return true; }

            let Some(name_attribute) = child_node.get_attribute("name", &self.manifest.string_pool) else { return true; };
            let ResValueType::String(ref string_value) = name_attribute.typed_value.data else { return true; };
            let Some(resolved_value) = string_value.resolve(&self.manifest.string_pool) else { return true; };
...
fn replace_package_references(
    ...
    let attributes_to_inspect = ["name", "authorities", "taskAffinity", "sharedUserId", "value", "scheme", "host"];

    for attribute_name in attributes_to_inspect {
        let Some(attribute_node) = element_node.get_attribute_mut(attribute_name, string_pool) else { continue; };
        ...
        match attribute_node.typed_value.data {
            ...
            ResValueType::Reference(ref table_reference) => {
                resolved_string_value = (|| -> Option<String> {
                    ...
                    let ResTableEntryValue::ResValue(ref actual_value) = resource_value.data else { return None; };
                    let ResValueType::String(ref string_reference) = actual_value.data.data else { return None; };
    ...
    for child_node in &mut element_node.children {
        replace_package_references(child_node, string_pool, resource_table, old_package_identity, new_package_identity);
    }
}
...
pub fn inject_and_build_apk(
    ...
    if patched_manifest_path.is_some() { active_files_to_inject.insert("AndroidManifest.xml".to_string()); }
    ...
    for archive_index in 0..zip_archive.len() {
        ...
        let short_file_name = Path::new(&internal_file_name).file_name().unwrap_or_default().to_string_lossy();
        if internal_file_name.starts_with("res/") {
            if short_file_name == "icon.png" && has_custom_icon { continue; }
            if short_file_name == "icon_foreground.png" && has_custom_foreground_icon { continue; }
    ...
    if let Some(manifest_path) = patched_manifest_path { inject_local_file(manifest_path, "AndroidManifest.xml", false)?; }
    ...
    if icons_directory.exists() {
        ...
        for (blueprint_file_name, size_xxxhdpi, size_xxhdpi, size_xhdpi, asset_exists) in icon_blueprints {
            ...
            for (target_folder_name, target_size) in target_resolutions {
                ...
                if properly_scaled_image.write_to(&mut memory_cursor, image::ImageFormat::Png).is_err() { continue; };

                let injection_options = zip::write::SimpleFileOptions::default().compression_method(zip::CompressionMethod::Stored);
                if zip_writer.start_file(&final_zip_path, injection_options).is_err() { continue; };
