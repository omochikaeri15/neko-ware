...
pub fn stream_pack_and_list(
    ...
    let parsed_standard_keys = if resolved_pack_type == cryptology::PackType::Standard {
        let decoded_key_bytes = hex::decode(&region_cryptology_key.key).map_err(|_| "Invalid Region Key Hex".to_string())?;
        let decoded_iv_bytes = hex::decode(&region_cryptology_key.iv).map_err(|_| "Invalid Region IV Hex".to_string())?;
    ...
    let generated_pack_file = File::create(&pack_output_path).map_err(|error| format!("Failed to create pack stream file: {}", error))?;
    ...
    for (active_file_path, _file_size) in valid_files_with_sizes.iter() {
        ...
        raw_file_data = cryptology::encrypt_chunk(&raw_file_data, resolved_pack_type, active_cipher_key, active_cipher_iv)
            .map_err(|error| format!("Encryption failed for {}: {}", extracted_filename, error))?;

        buffered_pack_writer.write_all(&raw_file_data).map_err(|error| format!("Failed to write to pack buffer: {}", error))?;
    ...
    fs::write(list_output_path, encrypted_list_bytes).map_err(|error| format!("Failed to write list file: {}", error))?;
