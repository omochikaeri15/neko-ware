...
impl UserKeys {
    ...
    pub fn print_status(&self, show_ui: bool) {
        if !show_ui {
            tracing::info!(keys = ?self, "Current user decryption keys");
            return;
        }
    ...
    pub fn prompt_interactive_load(show_ui: bool) -> Self {
        if !show_ui {
            tracing::error!("Interactive key loading requires standard UI mode.");
            std::process::exit(1);
        }
    ...
    pub fn to_nyanko_keys(&self) -> Result<nyanko::pack::cryptology::Keys, String> {
        ...
        let reference_tuples: Vec<(Region, &str, &str)> = collection_tuples
            .iter()
...
fn push_valid_key(
    tuple_vector: &mut Vec<(Region, String, String)>,
    ...
    if cleaned_key.len() != 32 {
        return;
    }
    if cleaned_iv.len() != 32 {
        return;
    }
